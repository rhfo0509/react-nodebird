...
요청을 할 때, 요청에 대한 응답이 성공적으로 올 때, 요청에 대한 응답이 실패할 때 <- 이렇게 3가지로 구성이 되어야 한다.
...
`loginAction` 한 번에 요청->성공, 요청->실패처럼 dispatch가 여러 번 일어나기 때문에 비동기 요청을 처리할 때 주로 사용된다.
...
사가 - 딜레이, 직접 구현해야 하는 thunk와 달리, 사가에서는 미리 만들어 제공
실수로 클릭을 두 번한 경우, thunk에서는 요청 요처 ㅇ다한ㅁ. sage 두번연속 이면 마지막것만tate
scroll 이벤트ㅔㅇ서도 
...
### all
fork나 call로 실행시킨 제너레이터 함수들을 병행적으로 동시에 실행되도록 한다.
...
2. `fork`와 `all` effect를 통해 `watchLogIn()` 실행
3. `take`에 의해 `LOG_IN_REQUEST` 액션이 dispatch될 때까지 기다림
4. `LOG_IN_REQUEST` 액션이 dispatch 될때 `logIn` 함수가 실행
5. 
